...
function getMotorLabel(value, motorRows, lang) {
  const row = (motorRows || []).find(item => String(item.value) === String(value));
...
async function init() {
  ...
  document.documentElement.lang = requestedLang === 'zh' ? 'zh-CN' : 'en';
  ...
  try {
    const [phonesRows, cameraRows, screenRows, wirelessRows, hardwareRows, chargingRows, chargingProtocolRows, memoryRows, storageRows, bandRows, refsRows, motorRows] = await Promise.all([
    ...
    const bands = bandRows
      .filter(r => Number(r.phone_id) === id)
      .map(r => ({ generation: r.generation || '', mode: r.mode || '', band: r.band || '' }));
    ...
    const bandGroupsHtml = bandGroups.length
      ...
      <div class="page-grid">
        <div class="panel hero">
          ...
          <div class="hero-sub">
            ...
            <span class="tag">${escapeHtml(phone.soc)}</span>
        ...
        <div class="panel">
          ...
          <div class="mini-stat-grid">
            <div class="mini-stat">
              <div class="mini-stat-label">${tr('length')}</div>
              <div class="mini-stat-value">${escapeHtml(phone.length_mm)} mm</div>
            </div>
            <div class="mini-stat">
              <div class="mini-stat-label">${tr('width')}</div>
              <div class="mini-stat-value">${escapeHtml(phone.width_mm)} mm</div>
            </div>
            <div class="mini-stat">
              <div class="mini-stat-label">${tr('thickness')}</div>
              <div class="mini-stat-value">${escapeHtml(phone.thickness_mm)} mm</div>
            </div>
            <div class="mini-stat">
              <div class="mini-stat-label">${tr('batteryCol')}</div>
              <div class="mini-stat-value">${escapeHtml(phone.battery_mah)} mAh</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="section-grid">
        <div class="panel">
          ...
          <div class="info-grid">
            ...
            ${infoItem(tr('soc'), escapeHtml(phone.soc))}
        ...
        <div class="panel">
          ...
          <div class="info-grid">
            ${charging ? `
              ${infoItem(tr('chargingPower'), `<span class="tag">${escapeHtml(charging.max_power_w)}W</span>`)}
              ${infoItem(tr('chargingProtocol'),
                chargingProtocolGroups.length
                  ? chargingProtocolGroups.map(group => `<div style="margin-bottom:6px;"><span class="tag">${escapeHtml(group.protocol)}</span>${group.powers.map(power => `<span class="tag">${power}W</span>`).join('')}</div>`).join('')
                  : `<span class="empty">${tr('detailNoData')}</span>`)}
            ` : `<div class="empty">${tr('detailNoData')}</div>`}
          </div>
        </div>
        ...
        <div class="panel">
          ...
          <div class="info-grid">
            ${screen ? `
              ${infoItem(tr('screenSize'), `<span class="tag">${escapeHtml(screen.screen_size_inch)}"</span>`)}
        ...
        <div class="panel">
          ...
          <div class="info-grid">
            ${wireless ? `${infoItem(tr('wifi'), `<span class="tag">${escapeHtml(wireless.wifi)}</span>`)}` : `<div class="empty">${tr('detailNoData')}</div>`}
          </div>
        </div>
        ...
        <div class="panel">
          ...
          <div class="info-grid">
            ${hardware ? `
              ${infoItem(tr('jack35'), `<span class="tag">${boolLabel(String(hardware.has_3_5mm).toLowerCase()==='true')}</span>`)}
              ...
              ${infoItem(tr('motor'), `<span class="tag">${escapeHtml(getMotorLabel(hardware.motor_type, motorRows, requestedLang))}</span>`)}
            ` : `<div class="empty">${tr('detailNoData')}</div>`}
          </div>
        </div>

        <div class="panel">
          <h2 class="card-title">${tr('memOptions')}</h2>
          <div class="list-block">${mem.length ? mem.map(v => `<span class="tag">${escapeHtml(v)}GB</span>`).join('') : `<div class="empty">${tr('detailNoData')}</div>`}</div>
        </div>

        <div class="panel">
          <h2 class="card-title">${tr('storageOptions')}</h2>
          <div class="list-block">${stor.length ? stor.map(v => `<span class="tag">${escapeHtml(v)}GB</span>`).join('') : `<div class="empty">${tr('detailNoData')}</div>`}</div>
        </div>

        <div class="panel full-span">
          <h2 class="card-title">${tr('referencesTitle')}</h2>
          <div class="refs-list">${refsHtml}</div>
        </div>
      </div>
    `;
  } catch (err) {
